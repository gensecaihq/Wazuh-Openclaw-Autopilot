...
### Fixed
- **web_fetch SSRF guard blocks localhost — agents cannot reach runtime API** (fixes #16): Even after tools are correctly registered via `alsoAllow`, OpenClaw's SSRF guard blocks `web_fetch` requests to private/localhost IPs by default. Agents calling `http://127.0.0.1:9090/api/...` get "Blocked hostname or private/internal/special-use IP address". Fix: added `"allowPrivateNetwork": true` to `tools.web.fetch` in all config files. This is safe because the runtime API runs on loopback only, and agents are trusted internal processes.
...
- **OpenRouter tool calling failure — agents output web_fetch as text** (fixes #14): OpenClaw's `openai-completions` provider path (used by OpenRouter) only sends tool definitions if the model has `toolUse: true` in its capability catalog. Without this metadata, models write `web_fetch http://...` as plain text instead of invoking it as a tool call (`stopReason: "stop"` instead of `"tool_use"`). Root cause: upstream OpenClaw bug ([openclaw#1866](https://github.com/openclaw/openclaw/issues/1866), [openclaw#8923](https://github.com/openclaw/openclaw/issues/8923)). Fix: installer now runs `openclaw models scan` to refresh model capability metadata after config generation. Documentation updated with manual workaround (run `openclaw models scan`) and fallback (use direct Anthropic API as primary model for reliable tool calling). Note: OpenClaw v2026.3.2 does NOT fix this upstream — the SSRF/DNS-pinning changes are unrelated.
