...
# Get specific case
curl http://127.0.0.1:9090/api/cases/CASE-2024-001

# Create new case (requires auth for non-localhost)
curl -X POST http://127.0.0.1:9090/api/cases \
  -H "Content-Type: application/json" \
  -d '{"case_id": "CASE-2024-001", "title": "Test Case", "severity": "high"}'

# Update case
curl -X PUT http://127.0.0.1:9090/api/cases/CASE-2024-001 \
  -H "Content-Type: application/json" \
  -d '{"status": "closed"}'
```
